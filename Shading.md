...
# 什么是 Shadow Mapping？
Shadow Mapping（阴影映射）是一种在实时渲染中生成动态阴影的技术，其核心思想是从光源的视角渲染场景，记录每个可见点的深度值到一张纹理（即阴影贴图，Shadow Map），然后在主相机视角渲染时，将当前像素的世界坐标变换到光源空间，比较该点与阴影贴图中存储的深度值：若该点的深度大于阴影贴图中对应点的深度，说明有更近的物体遮挡了光线，该点处于阴影中，否则被照亮。

实现流程分为两个 Pass：第一个 Pass，将相机放在光源位置（对于方向光使用正交投影，对于点光源使用立方体贴图），渲染整个场景但不写颜色，只输出深度值到阴影贴图；第二个 Pass，主相机正常渲染场景，在片段着色器中，计算当前片段在光源空间中的坐标 $(s, t, z_{\text{light}})$，用 $(s, t)$ 从阴影贴图中采样得到深度 $z_{\text{shadow}}$，若 $z_{\text{light}} > z_{\text{shadow}} + \text{bias}$，则判定为阴影区域，漫反射和高光项乘上一个阴影因子（如 0.2），否则保持原光照。

Shadow Mapping 的关键挑战包括**阴影贴图走样**（贴图分辨率有限导致锯齿状阴影边缘，可通过百分比渐近滤波 PCSS 或方差阴影映射 VSM 缓解）、**深度冲突**（因为浮点精度或采样误差，表面错误地自我遮蔽，通过添加深度偏移 bias 或使用背面剔除技术解决）、以及**软阴影模拟**（使用区域光源或多重采样结合 PCSS 来产生半影区）。由于其实现简单、性能可控且不依赖于几何复杂度，Shadow Mapping 成为游戏引擎中动态阴影的事实标准，几乎所有 3A 游戏都使用其变体（如级联阴影映射 CSM 用于大场景方向光阴影，立方体贴图阴影用于点光源全方向阴影）。

### Hard shadows vs. Soft shadows
硬阴影和软阴影的主要区别在于阴影边缘的过渡方式以及产生原因，二者分别对应于理想点光源/方向光和真实面光源的照明物理。

**硬阴影**由理想化的点光源或平行光（即光源尺寸为零或无限远且张角为零）产生，这种光源发出的光线只存在有无遮挡的两种状态，因此阴影边界是锐利的，没有任何半影区；在 Shadow Mapping 中，硬阴影表现为直接深度比较后得到的二值阴影因子（0或1），边缘的锯齿感明显（除非通过滤波抗锯齿）。

**软阴影**则源于具有一定面积的面光源（如太阳不是点光源而是有视面积的圆盘，或者室内的灯管、窗户），光源上不同区域被遮挡的程度不同导致阴影边缘呈现从全亮到全暗的渐变区域，这个渐变区域称为**半影**（Penumbra）。在半影区内，表面点只能“看到”光源的部分区域，因此光照强度线性或非线性地衰减至零；距离遮挡物越远，半影区越宽，阴影越模糊，这解释了为什么日食时地球上的阴影边缘有平滑的过渡。实时渲染中实现软阴影的方法包括：对 Shadow Map 进行多重采样滤波（如百分比渐近滤波 PCF，对周围多个纹素采样并平均得到 0~1 的阴影因子）、方差阴影映射（VSM，通过统计深度分布的方差来估计半影比例）、以及更高级的 Contact Hardening Soft Shadows（基于遮挡距离动态调整滤波核大小，模拟光源面积和遮挡距离对半影宽度的影响）。

物理上，软阴影的真实感远强于硬阴影，因为现实世界几乎不存在真正意义上的点光源（即使恒星也有有限视面积），但硬阴影计算快、存储小，因此在实时渲染中常根据光源类型和距离选择：远处的方向光（如太阳）通常使用软阴影的近似（如级联阴影映射加上PCF滤波），而极小的点光源（如手电筒）可能仍使用硬阴影或轻微柔边来平衡性能与效果。